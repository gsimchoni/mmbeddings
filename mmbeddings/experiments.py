...
from mmbeddings.models.embeddings import EmbeddingsMLP, HashingMLP, UnifiedEmbeddingsMLP
...
class REbeddings(Experiment):
    def __init__(self, exp_in, REbeddings_type, processing_fn=lambda x: x, plot_fn=None, growth_model=False, cf=False, tt=False, simulation_mode=True):
    ...
    def run(self):
        ...
        input_dim = self.get_input_dimension(X_train)
        model = self.model_class(self.exp_in, input_dim, self.last_layer_activation, self.growth_model, self.cf, self.tt)
