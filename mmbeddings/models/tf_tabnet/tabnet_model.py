...
from mmbeddings.models.tf_tabnet.utils import entmax15
...
class GLULayer(Layer):
    def __init__(
        ...
        if fc_layer:
            ...
            self.fc = Dense(self.units*2, use_bias=False)
        
        self.bn = BatchNormalization(virtual_batch_size=self.virtual_batch_size, 
...
class FeatureTransformer(Layer):
    def __init__(
            ...
            shared_glu_fc_layers: Optional[List[Dense]] = None, 
            units: int = 16, 
            virtual_batch_size: Optional[int] = None, 
            momentum: float = 0.98, 
            **kwargs
        ...
        shared_glu_fc_layers: List[Dense]
...
class AttentiveTransformer(Layer):
    def __init__(
        ...
        **kwargs
        ...
        self.fc = Dense(units, use_bias=False)

        self.bn = BatchNormalization(virtual_batch_size=virtual_batch_size, 
                                                     momentum=momentum)
...
class TabNetEncoder(Layer):
    def __init__(
        ...
        **kwargs
        ...
        self.initial_bn = BatchNormalization(momentum=self.momentum)

        # shared glu layers
        self.glu_dim = decision_dim + attention_dim
        self.shared_glu_fc_layers = list()
        for _ in range(n_shared_glus):
            self.shared_glu_fc_layers.append(Dense(units=self.glu_dim*2, use_bias=False))
...
class TabNetDecoder(Layer):
    ...
    def build(self, input_shape: tf.TensorShape):
        ...
        for _ in range(self.n_shared_glus):
            self.shared_glu_fc_layers.append(Dense(units=latent_dim*2, use_bias=False))
